...
void TouchControls::readSettings()
{
	...
	m_dpad_mode = g_settings->getBool("minesayme_dpad_mode");

	// MineSayme: crosshair mode overrides interaction style
	if (!g_settings->getBool("minesayme_crosshair_mode")) {
		m_interaction_style = TAP;
	}
...
void TouchControls::applyLayout(const ButtonLayout &layout)
{
	...
	// MineSayme: D-pad movement buttons (bottom-left, 2x2 grid)
	s32 dpad_size = m_button_size;
	s32 dpad_x = m_button_size * 0.5f;
	s32 dpad_y = m_screensize.Y - dpad_size * 2.8f;
	// up
	m_dpad_up = grab_gui_element<IGUIImage>(makeButtonDirect(dpad_up_id,
			recti(dpad_x + dpad_size, dpad_y, dpad_x + dpad_size * 2, dpad_y + dpad_size), false));
	// down
	m_dpad_down = grab_gui_element<IGUIImage>(makeButtonDirect(dpad_down_id,
			recti(dpad_x + dpad_size, dpad_y + dpad_size, dpad_x + dpad_size * 2, dpad_y + dpad_size * 2), false));
	// left
	m_dpad_left = grab_gui_element<IGUIImage>(makeButtonDirect(dpad_left_id,
			recti(dpad_x, dpad_y + dpad_size, dpad_x + dpad_size, dpad_y + dpad_size * 2), false));
	// right
	m_dpad_right = grab_gui_element<IGUIImage>(makeButtonDirect(dpad_right_id,
			recti(dpad_x + dpad_size * 2, dpad_y + dpad_size, dpad_x + dpad_size * 3, dpad_y + dpad_size * 2), false));
	loadButtonTexture(m_dpad_up.get(), "dpad_up.png");
	loadButtonTexture(m_dpad_down.get(), "dpad_down.png");
	loadButtonTexture(m_dpad_left.get(), "dpad_left.png");
	loadButtonTexture(m_dpad_right.get(), "dpad_right.png");
...
void TouchControls::handleReleaseEvent(size_t pointer_id)
{
	...
	// MineSayme: handle d-pad release
	auto dpad_it = m_dpad_active_pointers.find(pointer_id);
	if (dpad_it != m_dpad_active_pointers.end()) {
		emitGameKeyEvent(dpad_it->second, 0.0f);
		m_dpad_active_pointers.erase(dpad_it);
		return;
	}
...
void TouchControls::translateEvent(const SEvent &event)
{
	...
	if (event.TouchInput.Event == ETIE_PRESSED_DOWN) {
		...
		// MineSayme: handle d-pad movement buttons
		if (m_dpad_mode) {
			if (element == m_dpad_up.get()) {
				emitGameKeyEvent(KeyType::FORWARD, 1.0f);
				m_dpad_active_pointers[pointer_id] = KeyType::FORWARD;
				return;
			}
			if (element == m_dpad_down.get()) {
				emitGameKeyEvent(KeyType::BACKWARD, 1.0f);
				m_dpad_active_pointers[pointer_id] = KeyType::BACKWARD;
				return;
			}
			if (element == m_dpad_left.get()) {
				emitGameKeyEvent(KeyType::LEFT, 1.0f);
				m_dpad_active_pointers[pointer_id] = KeyType::LEFT;
				return;
			}
			if (element == m_dpad_right.get()) {
				emitGameKeyEvent(KeyType::RIGHT, 1.0f);
				m_dpad_active_pointers[pointer_id] = KeyType::RIGHT;
				return;
			}
		}
		...
		// MineSayme: skip joystick when d-pad mode is enabled
		if (!m_dpad_mode && ((m_fixed_joystick && dir_fixed.getLengthSQ() <= fixed_joystick_range_sq) ||
...
void TouchControls::updateVisibility()
{
	...
	m_joystick_btn_off->setVisible(regular_visible && !m_has_joystick_id && !m_dpad_mode);
	m_joystick_btn_bg->setVisible(regular_visible && m_has_joystick_id && !m_dpad_mode);
	m_joystick_btn_center->setVisible(regular_visible && m_has_joystick_id && !m_dpad_mode);

	// MineSayme: d-pad visibility
	if (m_dpad_up) m_dpad_up->setVisible(regular_visible && m_dpad_mode);
	if (m_dpad_down) m_dpad_down->setVisible(regular_visible && m_dpad_mode);
	if (m_dpad_left) m_dpad_left->setVisible(regular_visible && m_dpad_mode);
	if (m_dpad_right) m_dpad_right->setVisible(regular_visible && m_dpad_mode);
...
void TouchControls::releaseAll()
{
	...
	// MineSayme: release d-pad buttons
	for (auto &[pid, key] : m_dpad_active_pointers)
		emitGameKeyEvent(key, 0.0f);
	m_dpad_active_pointers.clear();
